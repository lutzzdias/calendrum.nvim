...
local ViewConfig = require("calendrum.view.config")
...
---@type FloatOptions
M.defaults = {
	size = { width = 0.5, height = 0.5 }, --Config.options.ui.size or Config.defaults.ui.size,
	style = "minimal",
	border = "rounded", -- Config.options.ui.border or Config.defaults.ui.border,
	zindex = 50,
	relative = "editor",
}
...
function M:init(opts)
	opts = opts or {}
	require("calendrum.view.colors").setup()

	self.opts = vim.tbl_deep_extend("force", self.defaults, opts)

	local width = self.size(vim.o.columns, self.opts.size.width)
	local height = self.size(vim.o.lines, self.opts.size.height)
